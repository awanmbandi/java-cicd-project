...
<div class="container">
    <h1>Your Cart</h1>
    <c:set var="hasItems" value="${not empty cart}" />
    <c:choose>
        <c:when test="${hasItems}">
            <table>
                ...
                <c:forEach var="entry" items="${cart}">
                    ...
                    <c:set var="grand" value="${grand + item.total}" scope="page" />
                </c:forEach>
                </tbody>
                <tfoot>
                    <tr><td colspan="3" class="right">Grand Total</td><td colspan="2">$${grand}</td></tr>
                </tfoot>
            </table>
            <div class="actions">
                <form method="post" action="${pageContext.request.contextPath}/cart" class="inline">
                ...
                <a class="link" href="${pageContext.request.contextPath}/products">Continue Shopping</a>
                <a class="button" href="${pageContext.request.contextPath}/checkout">Proceed to Checkout</a>
            </div>
        </c:when>
        <c:otherwise>
